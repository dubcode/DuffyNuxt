<template>
    <div id="blogs" class="row">
      <div class="ctr">
        <h6 class="main__title">Latest News</h6>
        <ul>
            ...
            <nuxt-link :to="{ path: '/blog/' + post.slug }">
              ...
              <div class="blogs__excerpt">
                <span class="blogs__crumb">
                  Latest News / 
                  <template v-if="post.categories.includes(61)">
                    Driving
                  </template>
                  <template v-else-if="post.categories.includes(21)">
                    Company
                  </template>
                  <template v-else-if="post.categories.includes(22)">
                    Industry
                  </template>
                  <template v-else-if="post.categories.includes(7)">
                    MOT
                  </template>
                  <template v-else-if="post.categories.includes(8)">
                    Service
                  </template>
                  <template v-else-if="post.categories.includes(9)">
                    Tyres
                  </template>
                  <template v-else>
                    Uncategorised
